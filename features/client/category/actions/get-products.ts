    ...
    try {
        ...
        if (categories.length > 0) {
            const categoryConditions = categories.map((category) =>
                eq(productDetails.category, category as any),
            );
        ...
        if (filters.min !== undefined && filters.max !== undefined) {
            conditions.push(
                between(productDetails.price, filters.min, filters.max),
            );
        }
...
const getAllCategories = async () => {
    try {
        const allCategories = await db
            .selectDistinct({ category: productDetails.category })
            .from(productDetails);
        if (!allCategories) {
            return undefined;
        }
...
const getAllBrands = async () => {
    try {
        const allBrands = await db
            .selectDistinct({ brand: productDetails.brand })
            .from(productDetails);
        if (!allBrands) {
            return undefined;
        }
