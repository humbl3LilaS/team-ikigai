...
const categories = [
    ...
    { name: "Storage Device", icon: "/flaticon/storage.png", className: "" },
...
const HomeCategory = () => {
  return (
    <section className='w-full mt-14'>
        ...
        <div className='max-w-full flex gap-5 justify-between py-2 overflow-x-scroll small-scrollbar'>
        {categories.map((item, index) => {
                    const encodedCategoryName = item.name.replace(
                        /[^a-zA-Z0-9]+/g,
                        "+",
                    );
                    return (
                    ...
                    <Link
                        href={`/category?category=${encodedCategoryName}`}
                        key={item.name + index}
                    >
                        <div key={index} className='flex justify-center items-center flex-col w-[150px] aspect-[1/1] p-2 shadow-lg rounded-md gap-2'>
